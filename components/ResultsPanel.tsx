...
import { DownloadIcon, GenerateIcon } from './IconComponents';
...
const ResultsPanel: React.FC<ResultsPanelProps> = ({ images, isLoading, error }) => {
    ...
    return (
        <div className="space-y-8">
            {Object.entries(groupedImages).map(([prompt, imageGroup]) => (
                <div key={prompt}>
                    <h3 className="text-lg font-semibold text-indigo-300 mb-4 sticky top-0 bg-slate-900/80 backdrop-blur-sm py-2 px-1 z-10">{prompt}</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {imageGroup.map((image) => (
                            <div key={image.id} className="group relative bg-slate-800 rounded-lg overflow-hidden shadow-lg transition-transform duration-300 hover:scale-105">
